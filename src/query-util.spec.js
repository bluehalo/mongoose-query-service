...
describe('query-util', () => {
	...
	describe('#toMongoose', () => {
		...
		it('should convert $date : {""} to new Date("")', () => {
			...
			var output = queryUtil.toMongoose(input);
	...
	describe('#getPage', () => {
		[{
			...
			it(test.name, () => {
				const actual = queryUtil.getPage(test.input);
				should(actual).equal(test.expected);
			});
		});
	});
	...
	describe('#getLimit', () => {
		...
		[{
			...
			it(test.name, () => {
				const actual = queryUtil.getLimit(test.inputQueryParams, test.inputMaxSize);
				should(actual).equal(test.expected);
			});
		});
	});
	...
	describe('#contains', () => {
		[{
			...
			it(test.name, () => {
				const actual = queryUtil.contains(test.inputArray, test.inputElement);
				should(actual).equal(test.expected);
			});
		});
	});
